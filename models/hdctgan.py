...
class HDCTGANSynthesizer(BaseSynthesizer):
    ...
    def fit(self, train_data, categorical_columns=tuple(), ordinal_columns=tuple()):
        ...
        # self.old_transformer = BGMTransformer()
        # self.old_transformer.fit(old_train_data, old_categorical_columns, ordinal_columns)
        # old_train_data = self.old_transformer.transform(old_train_data)
        # old_data_sampler = Sampler(old_train_data, self.old_transformer.output_info)
        # old_data_dim = self.old_transformer.output_dim
        # self.old_cond_generator = Cond(old_train_data, self.old_transformer.output_info)
        # loader = DataLoader(old_train_data, batch_size=self.batch_size, shuffle=True, drop_last=True)

        loader = DataLoader(train_data, batch_size=self.batch_size, shuffle=True, drop_last=True)
        ...
        #     data_dim*(1+self.num_one),
        #     self.compress_dims,
        #     self.embedding_dim
        # ).to(self.device)

        # self.old_encoder = Encoder(
        #     data_dim,
        #     self.compress_dims,
        #     self.embedding_dim
        # ).to(self.device)
        ...
        # old_optimizerD = optim.Adam(old_discriminator.parameters(), lr=2e-4, betas=(0.5, 0.9))
        ...
        for i in range(self.epochs):
            for id_ in range(steps_per_epoch):
                ...
                # real_cat = real_cat.clone().detach()
                # fake_cat = fake_cat.clone().detach()
                # real_cat.requires_grad = True
                # fake_cat.requires_grad = True
                # # fake_cat = self.old_encoder(fake_cat)
                # # real_cat = self.old_encoder(real_cat)
                # old_y_fake = old_discriminator(fake_cat)
                # old_y_real = old_discriminator(real_cat)
                # old_loss_d = -(torch.mean(old_y_real) - torch.mean(old_y_fake))
                # old_pen = calc_gradient_penalty(old_discriminator, real_cat, fake_cat, self.device)
                # old_optimizerD.zero_grad()
                # old_pen.backward(retain_graph=True)
                # old_loss_d.backward()
                # old_optimizerD.step()
