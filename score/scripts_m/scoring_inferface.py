...
#
# _, inDataFilename, inDataSpecsFilename, inGroundTruth1, inGroundTruth2 = sys.argv
...
################################################################################
# SCORING METHOD #1
def calcScore1_set3(inData, inGT, inDataSpecs):
    print('*********************************************************************')
    print('SCORING METHOD #1')
...
def calcScore1_set3_exist(inData, inGT, inDataSpecs):
    print('*********************************************************************')
    print('SCORING METHOD #1')
    ...
    for row in inGT:
        if row[0] == '@NEWCASE!':
            ...
            counts[testId] = {}
            columnSets.append([int(row[1]), int(row[2]), int(row[3])])
            statistics_dict[testId] = {}
            statistics_dict[testId]['elements'] = []
        else:
            test = counts[testId]
            key1 = row[0]
            key2 = row[1]
            ...
            if key2 not in test[key1]: test[key1][key2] = {}
            if key3 not in test[key1][key2]: test[key1][key2][key3] = count
            statistics_dict[testId]['elements'].append(float(count))
    ...
    for testId in counts:
        statistics_dict[testId]['collapse'] = []
        if testId not in solution:
            score -= 1.0
        else:
            for key1 in counts[testId]:
                for key2 in counts[testId][key1]:
                    for key3 in counts[testId][key1][key2]:
                        v2 = float(counts[testId][key1][key2][key3])
                        # v1 = float(solution[testId][key1][key2][key3]) / rowIndex
                        if key1 in solution[testId] and key2 in solution[testId][key1] and key3 in solution[testId][key1][
                            key2]:
                            v1 = v2
                            score += v2
                        else:
                            v1 = 0
                            statistics_dict[testId]['collapse'].append(v2)
                        score -= abs(v2 - v1)

    score *= 1000000.0 / 2 / len(counts.keys())

    print(f'SCORE #1 = {score}')
    return score, statistics_dict


def calcScore1_set4(inData, inGT, inDataSpecs):
    print('*********************************************************************')
    print('SCORING METHOD #1')
    ...
    for row in inGT:
        if row[0] == '@NEWCASE!':
            testId = len(columnSets)
            counts[testId] = {}
            columnSets.append([int(row[1]), int(row[2]), int(row[3]), int(row[4])])
        else:
            test = counts[testId]
            key1 = row[0]
            key2 = row[1]
            ...
            if key2 != '': key2 = int(key2)
            if key3 != '': key3 = int(key3)
            if key4 != '': key3 = int(key4)
            if count != '': float(count)
            if key1 not in test: test[key1] = {}
            if key2 not in test[key1]: test[key1][key2] = {}
            if key3 not in test[key1][key2]: test[key1][key2][key3] = {}
            if key4 not in test[key1][key2]: test[key1][key2][key3][key4] = count
    ...
    for row in inData:
        ...
        for index in range(len(columnSets)):
            if index not in solution: solution[index] = {}
            cset = columnSets[index]
            if cset[2] < len(row) and cset[1] < len(row) and cset[0] < len(row) and cset[3] < len(row):
                solution[index] = countRow(row, solution[index], cset, 0)
            else:
                solution[index] = {'outrange': {'outrange': {'outrange': rowIndex}}}
    ...
    for testId in counts:
        if testId not in solution:
            ...
            for key1 in solution[testId]:
                for key2 in solution[testId][key1]:
                    for key3 in solution[testId][key1][key2]:
                        for key4 in solution[testId][key1][key2][key3]:
                            v1 = float(solution[testId][key1][key2][key3][key4]) / rowIndex
                            if key1 in counts[testId] and key2 in counts[testId][key1] and key3 in counts[testId][key1][
                                key2] and key4 in counts[testId][key1][key2][key3]:
                                v2 = float(counts[testId][key1][key2][key3][key4])
                                score += v2
                            else:
                                v2 = 0
                            score -= abs(v2 - v1)
...
def calcScore1_set4_exist(inData, inGT, inDataSpecs):
    print('*********************************************************************')
    print('SCORING METHOD #1')
    ...
    for testId in counts:
        if testId not in solution:
            ...
            for key1 in counts[testId]:
                for key2 in counts[testId][key1]:
                    for key3 in counts[testId][key1][key2]:
                        for key4 in counts[testId][key1][key2][key3]:
                            # v1 = float(solution[testId][key1][key2][key3][key4]) / rowIndex
                            v2 = float(counts[testId][key1][key2][key3][key4])
                            if key1 in solution[testId] and key2 in solution[testId][key1] and key3 in solution[testId][key1][
                                key2] and key4 in solution[testId]:
                                v1 = v2
                                score += v2
                            else:
                                v1 = 0
...
def calcScore1_set1(inData, inGT, inDataSpecs):
    print('*********************************************************************')
    print('SCORING METHOD #1')
    ...
    for row in inGT:
        if row[0] == '@NEWCASE!':
            ...
            if key1 != '': key1 = int(key1)
            if count != '': float(count)
            if key1 not in test: test[key1] = count
    ...
    for testId in counts:
        if testId not in solution:
            ...
            for key1 in solution[testId]:
                v1 = float(solution[testId][key1]) / rowIndex
                if key1 in counts[testId]:
                    v2 = float(counts[testId][key1])
                    score += v2
                else:
                    v2 = 0
                score -= abs(v2 - v1)
...
def calcScore1_set1_exist(inData, inGT, inDataSpecs):
    print('*********************************************************************')
    print('SCORING METHOD #1')
...
def calcScore1_set2(inData, inGT, inDataSpecs):
    print('*********************************************************************')
    print('SCORING METHOD #1')
    ...
    for row in inGT:
        if row[0] == '@NEWCASE!':
            testId = len(columnSets)
            counts[testId] = {}
            columnSets.append([int(row[1]), int(row[2])])
        else:
            test = counts[testId]
            key1 = row[0]
            key2 = row[1]
            ...
            if count != '': float(count)
            if key1 not in test: test[key1] = {}
            if key2 not in test[key1]: test[key1][key2] = count
    ...
    for row in inData:
        ...
        for index in range(len(columnSets)):
            if index not in solution: solution[index] = {}
            cset = columnSets[index]
            if cset[1] < len(row) and cset[0] < len(row):
                solution[index] = countRow(row, solution[index], cset, 0)
            else:
                solution[index] = {'outrange': {'outrange': {'outrange': rowIndex}}}
    ...
    for testId in counts:
        if testId not in solution:
            ...
            for key1 in solution[testId]:
                for key2 in solution[testId][key1]:
                    v1 = float(solution[testId][key1][key2]) / rowIndex
                    if key1 in counts[testId] and key2 in counts[testId][key1]:
                        v2 = float(counts[testId][key1][key2])
                        score += v2
                    else:
                        v2 = 0
                    score -= abs(v2 - v1)
...
def calcScore1_set2_exist(inDataFilename, inGroundTruth1, inDataSpecsFilename):
    print('*********************************************************************')
    print('SCORING METHOD #1 minority')
        ...
        if testId not in solution:
            ...
            for key1 in counts[testId]:
                for key2 in counts[testId][key1]:
                    ...
                    if key1 in solution[testId] and key2 in solution[testId][key1]:
                        v1 = float(solution[testId][key1][key2]) /rowIndex
                        num_existed += 1
                    else:
                        v1 = 0.0
    ...
    score = 1e6 * max(0, (1 + sqrt(score/ num_minors) / log(1e-3)))

    print(f'SCORE #1 = {score}')
    return score, float(num_existed)/num_minors
        ...
        for testCaseId in range(len(testCases)):
            ...
            for item in testCases[testCaseId][1:]:
                ...
                if columnId >= len(row):
                    ...
                    if int(float(row[columnId])) not in item[2:]:
                        count = False
                        break
                else:
                    if item[2] == '':
...
#
# f = open(SR_FOLDER, "a")
# f.write(inDataFilename + ", " + inDataSpecsFilename + ", " + inGroundTruth1 + ", " + inGroundTruth2 + ", " + str(score1)
#         + ", " + str(score2) + ", " + str(score) + "\n")
# f.close()
# print("Saved to %s" % SR_FOLDER)
