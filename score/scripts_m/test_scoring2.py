...
list_args = sys.argv
if len(list_args) == 6:
    _, inDataFileFolder, inDataSpecsFilename, inGroundTruth1, inGroundTruth2, SR_FOLDER = list_args
else:
    _, inDataFileFolder, inDataSpecsFilename, inGroundTruth1, inGroundTruth2 = list_args
    SR_FOLDER = "score_results/log_score.txt"


################################################################################
# SCORING METHOD #1
def calcScore1(data_file):
    print('*********************************************************************')
...
list_files = glob.glob(os.path.join(inDataFileFolder, "*.csv"))
for file_name_path in list_files:
    score1 = calcScore1(file_name_path)
    score2 = calcScore2(file_name_path)
    file_base_name = os.path.basename(file_name_path)
    score = (score1 + score2) / 2
    score_path = os.path.join(SR_FOLDER, file_name_path)
    print('OVERAL SCORE =', score)

    f = open(SR_FOLDER, "a")
    f.write(file_base_name + ", " + os.path.basename(inDataSpecsFilename) + ", "
            + os.path.basename(inGroundTruth1)
            + ", " + os.path.basename(inGroundTruth2) + ", " + str(score1)
